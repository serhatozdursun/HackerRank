...
public class MiniMaxSum {
    ...
    public static void miniMaxSum(List<Integer> arr) {
        ...
        var maxValueList = sortedList.subList(1,sortedList.size()).
                stream().map(BigInteger::valueOf).reduce(BigInteger::add);
        var minValueList = sortedList
                .subList(0, sortedList.size()-1) .stream().map(BigInteger::valueOf).reduce( BigInteger::add);
